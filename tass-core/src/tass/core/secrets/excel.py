import openpyxl
from . import manager as secrets
...
class Excel(secrets.DataSource):
    ...
    def _load_datasource(self, config):
        ...
        _collections = [_c for _c in _all_collections
                        if _c['name'] in _source['collections']]
    ...
    def _load_collections(self, source, collections, entry_sets):
        ...
        for coll in collections:
            _name = coll['name']
            _entry_set = [se for se in entry_sets
                          if se['name'] == coll['entry-set']][0]
    ...
    class Sheet(secrets.Collection):
        ...
        def _load_entries(self, collection, entry_set):
            ...
            for index, row in enumerate(
                                    collection.iter_rows(min_row=start_row),
                                    start_row):
                entry = {"row": index}

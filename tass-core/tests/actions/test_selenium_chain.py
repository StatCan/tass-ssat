            ...
            for browser in self.drivers:
                ...
                with self.subTest(browser=browser[1].__name__):
                     try:
                        ...
                        self.assertTrue(bool(driver.chain().w3c_actions.devices[2].actions))
                        chain.perform(driver)
                        time.sleep(2)
                        scrolled = driver().execute_script("return window.pageYOffset")
            ...
            for browser in self.drivers:
                ...
                with self.subTest(browser=browser[1].__name__):
                     try:
                        ...
                        locator = {"by": "id", "value": "dropdown"}
                        chain.scroll(driver, locator=locator, deltax=0, deltay=scrolled*-1)
                        self.assertTrue(bool(driver.chain().w3c_actions.devices[2].actions))
                        chain.perform(driver)
                        time.sleep(2)
                        scrolled2 = driver().execute_script("return window.pageYOffset")

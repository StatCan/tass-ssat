...
class TassTestrailReporter(TestrailReporter):
    ...
    def report(self, run, *args, **kwargs):
        ...
        for case in run.completed_cases:
            id = self._get_case_id(
                      case.uuid, run.var('testrail')['project_id']
                      )
            status = self._get_status_id(case.status)
            if not id:
                log.warning("Unable to report on case: %s", id)
                continue
            elif not status:
                log.warning(
                    "Case was not started. Skipping report for: %s", id
                    )
                continue
            steps = case.steps
            tr_steps = [{
    ...
    def _get_case_id(self, uuid, project_id):
        ...
        try:
            ...
            log.info(
                (
                    "Case with uuid: %s not found in memory. "
                    "Loading cases from Testrail"
                ),
                uuid
                )
